...
export default function ClientsPreview() {
  ...
  const TestimonialTrack = ({
    ...
    <div dir="ltr" className="group relative overflow-hidden">
      ...
      <div
        className="flex w-max flex-row items-stretch gap-5 py-3 group-hover:[animation-play-state:paused]"
        style={{
          animation: `marquee ${duration}s linear infinite`,
          animationDirection: reverse ? 'reverse' : 'normal',
        }}
      >
        {[...items, ...items].map((raw, i) => (
          <TestimonialCard
            ...
            raw={raw}
            locale={locale}
            readLabel={t.testimonials.readOnGoogle}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <section className="section-dark relative overflow-hidden py-16 md:py-24">
      ...
      {/* Full-width logo marquees — outside Container so logos scroll edge to edge */}
      <div className="mt-4 space-y-2">
        <LogoTrack items={logoRow1} duration={140} />
        {logoRow2.length > 0 && <LogoTrack items={logoRow2} reverse duration={160} />}
      </div>
